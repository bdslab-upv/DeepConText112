"""
DESCRIPTION: specificity metric avalanche.
...
class Specificity(Metric[float]):
    ...
    def update(self, predicted_y: Tensor, true_y: Tensor,
        ...
        if isinstance(task_labels, int):
            ...
            specificity = posneg.true_negatives/(posneg.true_negatives + posneg.false_positives) \
                if (posneg.true_negatives + posneg.false_positives) != 0 else float('nan')

            # Arrangement
            self._mean_specificity[task_labels].update(specificity)
    ...
    def result(self, task_label=None) -> Dict[int, float]:
        ...
        :param task_label: if None, return the entire dictionary of specificitys
            for each task. Otherwise return the dictionary
            `{task_label: specificity}`.
        :return: A dict of running specificitys for each task label,
...
class SpecificityPluginMetric(GenericPluginMetric[float]):
    """
    Base class for all specificitys plugin metrics
    """
